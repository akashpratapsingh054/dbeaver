...
public class SQLServerTableColumnManager extends SQLTableColumnManager<SQLServerTableColumn, SQLServerTableBase> implements DBEStructEditor<SQLServerTableColumn>, DBEObjectRenamer<SQLServerTableColumn> {
    ...
    protected void addObjectModifyActions(DBRProgressMonitor monitor, DBCExecutionContext executionContext, List<DBEPersistAction> actionList, ObjectChangeCommand command, Map<String, Object> options)
    {
        ...
        if (column.isPersisted() && command.hasProperty("defaultValue")) {
            ...
            addDropConstraintsActions(monitor, command.getObject(), actionList);
    ...
    protected void addObjectDeleteActions(DBRProgressMonitor monitor, DBCExecutionContext executionContext, List<DBEPersistAction> actions, SQLObjectEditor<SQLServerTableColumn, SQLServerTableBase>.ObjectDeleteCommand command, Map<String, Object> options) throws DBException {
        addDropConstraintsActions(monitor, command.getObject(), actions);
    ...
    private static void addDropConstraintsActions(@NotNull DBRProgressMonitor monitor, @NotNull SQLServerTableColumn column, @NotNull List<DBEPersistAction> actions) {
        String defaultConstraintName = null;

        try (JDBCSession session = DBUtils.openMetaSession(monitor, column, "Read default constraint")) {
            defaultConstraintName = JDBCUtils.queryString(session, "SELECT name FROM " +
        ...
        if (defaultConstraintName != null) {
            actions.add(new SQLDatabasePersistAction(
                ...
                "ALTER TABLE " + column.getTable().getFullyQualifiedName(DBPEvaluationContext.DDL) + " DROP CONSTRAINT " + DBUtils.getQuotedIdentifier(column.getDataSource(), defaultConstraintName)
            ));
        }
    }
}
